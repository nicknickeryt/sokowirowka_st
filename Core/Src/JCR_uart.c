...
void JCR_uart_Process() {
    ...
    if (now - lastTick >= 500) {
        ...
        static char msg[64];
        snprintf(msg, sizeof(msg), "WMM%4lu.JMM%4lu.MUG%u.WTG%4u.JTG%4u.WCV%4u.JCV%4u\r\n",
                 JCR_sensor_GetDistanceWater(), JCR_sensor_GetDistanceJuice(),
                 JCR_Mug_IsPresent(), JCR_Containers_GetTargetMlWater(), JCR_Containers_GetTargetMlJuice(), (uint8_t) JCR_Containers_GetVolumeDeltaCcmWater(), (uint8_t) JCR_Containers_GetVolumeDeltaCcmJuice());
...
void JCR_uart_Callback(UART_HandleTypeDef* huart, uint16_t size) {
    ...
    if (strncmp((char*)rxBuffer, startCommand, strlen(startCommand)) == 0) {
        HAL_UART_Transmit_IT(&huart1, (uint8_t*)msg, strlen(msg));
        startPressed = true;
    } else if (strncmp((char*)rxBuffer, stopCommand, strlen(stopCommand)) ==
               0) {
        HAL_UART_Transmit_IT(&huart1, (uint8_t*)msg, strlen(msg));
        startPressed = false;
