...
void JCR_LcdPrint_Process() {
  ...
  if (settingMode == MODE_STATUS) {
    ...
    float juiceRatio = 1.0f - ((float)(juiceLevel - CONTAINER_MIN_MM) /
                               (CONTAINER_MAX_MM - CONTAINER_MIN_MM));
    float waterRatio = 1.0f - ((float)(waterLevel - CONTAINER_MIN_MM) /
                               (CONTAINER_MAX_MM - CONTAINER_MIN_MM));
...
void JCR_Lcd_SwitchSettingMode() {
  settingMode = settingMode == MODE_STATUS ? MODE_JUICE : settingMode + 1;
}
...
uint8_t JCR_Lcd_GetSettingMode() { return settingMode; }